...
def run():
    ...
    def undo(ntimes):
        '''Helper function to carry out valid undo command'''
        while ntimes != 0:
            try:
                ntimes = ntimes - 1
                ...
                elif (commandpop[0:1] == ['enrol'] and 
                      str(resultcheck).split()[0] not in 
                      ['ERROR:', 'Nothing']):
                    database.drop(commandpop[1], commandpop[2])
                elif (commandpop[0:1] == ['drop'] and 
                      str(resultcheck).split()[0] not in 
                      ['ERROR:', 'Nothing']):
    ...
    while True:
        ...
        if command == []:
            ...
            if len(command) == 2:
                ...
                ntimes = 1            
            undo(ntimes) #Call Helper function to carry out valid undo command
